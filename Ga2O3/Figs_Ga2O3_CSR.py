...
# custom imports
import Ga2O3_fun
import Ga2O3_type_peak_assignments
...
def CSR_plot(run_number, comp_csr_fig_idx, spec_fig_num, multi_fig_num):
    # Read in data
    epos = Ga2O3_fun.load_epos(run_number=run_number, 
                             epos_trim=[5000, 5000],
                             fig_idx=plt.figure().number)
    
    pk_data = Ga2O3_type_peak_assignments.Ga2O3()
    bg_rois=[[0.4,0.9]]
    
    pk_params, glob_bg_param, Ga1p_idxs, Ga2p_idxs = Ga2O3_fun.fit_spectrum(
            epos=epos, 
            pk_data=pk_data, 
            peak_height_fraction=0.1, 
            bg_rois=bg_rois)
    
    cts, compositions, is_peak = Ga2O3_fun.count_and_get_compositions(
            epos=epos, 
            pk_data=pk_data,
            pk_params=pk_params, 
    ...
    xs, ys_sm = Ga2O3_fun.bin_and_smooth_spectrum(epos=epos,
                                                user_roi=[0,150],
                                                bin_wid_mDa=30,
                                                smooth_wid_mDa=-1)
    ...
    Ga2O3_fun.create_det_hit_plots(epos,pk_data,pk_params,fig_idx = plt.figure().number)
    
    # Find the pole center and show it
    ax = plt.gcf().get_axes()[0]
    m2q_roi = [3, 100]
    sel_idxs = np.where((epos['m2q']>m2q_roi[0]) & (epos['m2q']<m2q_roi[1]))
    xc,yc = Ga2O3_fun.mean_shift(epos['x_det'][sel_idxs],epos['y_det'][sel_idxs])
    ...
    time_chunk_centers,r_centers,idxs_list = Ga2O3_fun.chop_data_rad_and_time(epos,
                                                                            [xc[-1],
                                                                             yc[-1]],
                                                                             time_chunk_size=2.2**16,
                                                                             N_ann_chunks=3)
...
CSR_plot(run_number='R20_28218_862pA',
         comp_csr_fig_idx=csr_fig.number,
         spec_fig_num=spec_fig.number,
         multi_fig_num=multi_fig.number) # 400 nm Al filter
#CSR_plot(run_number='R20_18156',
#         comp_csr_fig_idx=csr_fig.number,
#         spec_fig_num=spec_fig.number,
#         multi_fig_num=multi_fig.number)# 600 nm Al filter
#CSR_plot(run_number='R20_18160',
#         comp_csr_fig_idx=csr_fig.number,
#         spec_fig_num=spec_fig.number,
#         multi_fig_num=multi_fig.number) # 1 um Al filter
#CSR_plot(run_number='R20_18161',
#         comp_csr_fig_idx=csr_fig.number,
#         spec_fig_num=spec_fig.number,
#         multi_fig_num=multi_fig.number) # 1.2 um Al filter
#CSR_plot(run_number='R20_18157',
#         comp_csr_fig_idx=csr_fig.number,
#         spec_fig_num=spec_fig.number,
#         multi_fig_num=multi_fig.number) # 2 um Al filter
...
#csr_fig.savefig('Ga2O3_CSR_plot.pdf')
#csr_fig.savefig('Ga2O3_CSR_plot.jpg', dpi=300)
...
#multi_fig.savefig('Ga2O3_CSR_multi_plot.pdf')
#multi_fig.savefig('Ga2O3_CSR_multi_plot.jpg', dpi=300)
...
#spec_fig.savefig('Ga2O3_full_CSR_spectrum.pdf')
#spec_fig.savefig('Ga2O3_full_CSR_spectrum.jpg', dpi=300)
