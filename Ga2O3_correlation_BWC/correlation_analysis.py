...
#fn = r'C:\Users\capli\Google Drive\NIST\pos_and_epos_files\Diercks_Mar_3_2021_GaN\R45_00187-v01.epos'
fn = r'R44_03672_vbmq_corr.epos'

# fn = r'C:\Users\bwc\Documents\NetBeansProjects\R20_28215\recons\recon-v01\default\R20_28215-v01.epos'
...
tmp_idxs = np.where(epos['ipp'] == 3)[0]
triple_idxs = np.ravel(np.column_stack([tmp_idxs+i for i in range(3)]))
...
plotting_stuff.plot_histo(epos['m2q'],fig_idx=1,user_xlim=[0,500])


tof_corr = epos['tof']*tof_bcorr_fac*tof_vcorr_fac


plotting_stuff.plot_TOF_vs_time(tof_corr[sing_idxs],epos[sing_idxs],1, user_ylim=[400, 550])
plotting_stuff.plot_TOF_vs_time(tof_corr[doub_idxs],epos[doub_idxs],1, user_ylim=[400, 550])
plotting_stuff.plot_TOF_vs_time(tof_corr[triple_idxs],epos[triple_idxs],1, user_ylim=[400, 550])



plotting_stuff.plot_histo(tof_corr[sing_idxs],1,user_label='sing', user_xlim=[100,1000], user_bin_width=0.2)
plotting_stuff.plot_histo(tof_corr[doub_idxs],1,user_label='doub', clearFigure=False, user_xlim=[100,1000], user_bin_width=0.2)
plotting_stuff.plot_histo(tof_corr[triple_idxs],1,user_label='trip', clearFigure=False, user_xlim=[100,1000], user_bin_width=0.2)


idxs = np.where((epos_d_vb['m2q']>45) & (epos_d_vb['m2q']<49))[0]
plt.figure()
plt.scatter(epos_d_vb['x_det'][idxs[0:100000]],epos_d_vb['y_det'][idxs[0:100000]],marker=',',s=1,alpha=.1)
...
edges, ch = corrhist(epos_d_vb,roi = [0, 2000], delta=1)
...
# Make a correlation histogram of all
epos_vb = epos.copy()
epos_vb['tof'] = mod_full_vb_correction(epos,p_volt,p_bowl)

edges, ch = corrhist(epos_vb,roi = [0, 4000], delta=1)
centers = (edges[1:]+edges[:-1])/2.0
...
epos_b = epos.copy()
epos_b['tof'] = voltage_and_bowl.mod_geometric_bowl_correction(p_bowl,epos['tof'],epos['x_det'],epos['y_det'])




idxs3 = np.where(epos_vb['ipp']==3)[0]
idxs3 = np.sort(np.r_[idxs3,idxs3+1,idxs3+2])

epos_vb_3 = epos_vb[idxs3]

edges, ch = corrhist(epos_vb_3,roi = [0, 4000], delta=1)
centers = (edges[1:]+edges[:-1])/2.0

fig2 = plt.figure(num=5)
fig2.clf()
ax2 = fig2.gca()   
plot_2d_histo(ax2, ch, edges, edges, scale='log')
ax2.axis('equal')
...
t1 = epos_vb['tof'][idxs3]
t2 = epos_vb['tof'][idxs3+1]
t3 = epos_vb['tof'][idxs3+2]

sub_idxs = np.where(((t1>510) & (t1<530)) | ((t2>510) & (t2<530)) | ((t3>510) & (t3<530)))[0]



fig = plt.figure()
ax = fig.add_subplot(projection='3d')
ax.scatter(t1[sub_idxs],
           t2[sub_idxs],
           t3[sub_idxs],
           marker = 'o',
           color='k',
...
tofin = epos_d['tof'].copy()
vcorr_fac_in = tof_vcorr_fac_d
bcorr_fac_in = tof_bcorr_fac_d
...
N_SIGMA_PTS = 1024
DEL_SIGMA_PTS = 5000/N_SIGMA_PTS
