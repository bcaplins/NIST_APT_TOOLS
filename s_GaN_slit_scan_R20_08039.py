...
ref_fn = r'C:\Users\capli\Google Drive\NIST\pos_and_epos_files\GaN_BeamScans_May192021\R20_08039-v01.epos'
...
fn = r'C:\Users\capli\Google Drive\NIST\pos_and_epos_files\GaN_BeamScans_May192021\R20_08039-v01.epos'
...
offset = 14
for i in range(16):
    ax.plot(offset+i*120*np.ones(2),[0,1200],'--k')

fig = plt.figure()
ax = fig.gca()    
ax.hist(wall_time,bins=np.linspace(offset,15*120+13,4*1024))

for i in range(16):
    ax.plot(13+i*120*np.ones(2),[0,1200],'--k')
...
for slit_idx in range(slit_positions.size):
    chunk_idxs = np.where((wall_time>=(120*slit_idx+offset+padding)) & (wall_time<=(120*(slit_idx+1)+offset-padding)))[0]
    ...
    plt.savefig('HITMAP_R20_08039_Slit_Pos_'+str(slit_positions[slit_idx])+'.png', dpi=150)

    Ga_ct = np.where((epos[chunk_idxs]['m2q']>roi[0]) & (epos[chunk_idxs]['m2q']<roi[1])[0].size)[0].size
    Ga_cts.append(Ga_ct)
...
fig = plt.figure()
ax = fig.gca()  
ax.bar(slit_positions, np.array(sum_cts),0.45,label='All')
ax.bar(slit_positions, np.array(Ga_cts),0.45,label='10 m/z to 100 m/z')
