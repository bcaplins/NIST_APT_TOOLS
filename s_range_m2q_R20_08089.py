...
fn = r"C:\Users\bwc\Documents\NetBeansProjects\R20_08089\recons\recon-v01\default\R20_08089-v01.epos"
...
roi_event_idxs = np.arange(300000,epos.size)
...
labs = ["1x200","2x200","3x200","1x1000","1x2000","1x1000+1x200","1x2000 r","1x1000 r",
        "3x200 r","2x200 r","1x200 r"]


full_bg_est = epos['m2q'][(epos['m2q']>125) & (epos['m2q']<150)].size
...
counter = 0
for the_roi in the_rois:
    
    
    print('######################################################')
    print(str(the_roi))          
    sub_epos = epos[(1000*the_roi[0]):(1000*the_roi[1])]
    
    sub_bg_est = sub_epos['m2q'][(sub_epos['m2q']>125) & (sub_epos['m2q']<150)].size
    
    # Count the peaks, local bg, and global bg
    cts = ppd.do_counting(sub_epos,pk_params,glob_bg_param*(sub_bg_est/full_bg_est))
    ...
    # Plot all the things
    xs, ys = bin_dat(sub_epos['m2q'],user_roi=[0.5, 120],isBinAligned=True)
    #ys_sm = ppd.do_smooth_with_gaussian(ys,10)
    ys_sm = ppd.moving_average(ys,30)
    
    
    
    ax.plot(xs,ys_sm*(10**counter),label=labs[counter])
#    plt.pause(1)
...
labs = ["1x1000+1x200","1x2000 r","1x1000 r",
        "3x200 r","2x200 r","1x200 r"]
...
for the_roi in the_rois:
    ...
    ys_sc = (ys_sm+0.1)/np.sum(ys_sm)
    
    
#    ax.plot(xs,ys_sc*(10**-counter),label=labs[counter])
    ax.plot(xs,ys_sc,label=labs[counter])
#    plt.pause(1)
