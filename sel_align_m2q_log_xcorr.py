...
def __main__():
    ...
    TEST_PEAK = 71;
    ax = plotting_stuff.plot_TOF_vs_time(epos['m2q'],epos,111,clearFigure=True,user_ylim=[0,100])
    ax.plot(pointwise_scales*TEST_PEAK)
    
    plotting_stuff.plot_TOF_vs_time(m2q_corr,epos,222,clearFigure=True,user_ylim=[0,100])
    
    
    # Plot histograms uncorrected and corrected
    plotting_stuff.plot_histo(m2q_corr,333,user_xlim=[0, 100])
    plotting_stuff.plot_histo(epos['m2q'],333,user_xlim=[0, 100],clearFigure=False)
